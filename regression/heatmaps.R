...
processData <- function(data){
  ...
  data$index <- 1
  data$index <- ave(data$index, cumsum(c(F, diff(data$ts) > 0)), FUN=seq_along)
  data$freq<-ave( as.numeric(data[[1]]), data[["ts"]] , FUN=length)
  ...
  data<-melt(data, id.vars = c("FN","FP","TN","TP","ts","t","index","freq"))
...
for (v in unique(data$variable)){
  ...
  plot<-ggplot(datatmp,aes(ts,index))+geom_tile(aes(fill=value))+
